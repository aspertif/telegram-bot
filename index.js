...
// Gestisci il comando /start
bot.onText(/\/spataro/, (msg) => {
  const chatId = msg.chat.id;
  bot.sendMessage(chatId, 'ti cinghio.');
});
// Gestisci il comando /start
bot.onText(/\/black eagle/, (msg) => {
  const chatId = msg.chat.id;
  bot.sendMessage(chatId, 'COW COW');
});

// Gestisci il comando /start
bot.onText(/\/suria/, (msg) => {  
  const chatId = msg.chat.id;
  bot.sendMessage(chatId, 'ti metto tre!!'); 
});  
...
bot.on('message', (msg) => {
  ...
  if (msg.text && (msg.text.startsWith('/start') || 
                  msg.text.startsWith('/spataro') ||      
                  msg.text.startsWith('/black eagle') ||  
                  msg.text.startsWith('/suria') ||         
